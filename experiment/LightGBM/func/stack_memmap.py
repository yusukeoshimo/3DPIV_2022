...
def stack_memmap(memmap_path_list, saved_dir, video_height, video_width):
    ...
    # memmapをくっつける
    new_memmap = np.memmap(stack_memmap_path, dtype='float32', mode='w+', shape=(total_size)).reshape(-1, video_height, video_width) # くっつけてできる新しいmemmap
    read_data_num = 0 # 読み込み済みのデータ数
    for memmap_path in memmap_path_list: # くっつけたいmemmapのパスが入ったリストをfor文で回す
        original_memmap = np.memmap(memmap_path, dtype='float32', mode='r').reshape(-1, video_height, video_width) # for文で回ってきたmemmapを読み込む
