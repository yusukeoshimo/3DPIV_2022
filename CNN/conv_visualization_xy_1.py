...
fig_h = conv_weight.shape[3]
fig_w = 8
fig = plt.figure()
plt.subplots_adjust(wspace=0.3, hspace=0)
for channel in range(fig_h):
    for i in range(fig_w):
        plt.subplot(fig_h,fig_w,channel*fig_w+i+1)
        plt.imshow(cv2.cvtColor(conv_weight[i,:,:,channel], cv2.COLOR_BGR2RGB))
        plt.tick_params(bottom=False, left=False, right=False, top=False)
        plt.tick_params(labelbottom=False, labelleft=False, labelright=False, labeltop=False)
        if channel == 0:
            plt.title(f'kernel {i+1}', fontsize=8)
        if i == 0:
            plt.ylabel(f'channel {channel+1}', fontsize=8)
plt.tight_layout()
